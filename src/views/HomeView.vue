<template>
    <div class="home-view">
        ...
        >
            ...
            <p>
                解析与下载依赖你本地网络可正常访问视频站点（如 B 站、YouTube
                等）。若你的网络无法在浏览器中打开这些站点，OmniFetch
                将无法解析视频信息，也无法下载视频。建议先确认可在浏览器中打开目标链接后再使用本工具。
            </p>
        </t-alert>
        ...
        >
            <template #title>
                检测到二进制有可用更新（{{
                    store.binaryUpdateNotice.map((u) => u.name).join('、')
                }}）
            </template>
            <p>
            ...
            <p v-if="!store.binaryUpdateResult">
                ...
                <t-button
                    ...
                    variant="text"
                    :disabled="store.isUpdatingBinaries"
                    @click="store.dismissBinaryUpdate()"
                >
                    忽略
                </t-button>
            </p>
            ...
            <p v-else class="update-fail">
                更新失败（{{
                    store.binaryUpdateResult.failed.join('、')
                }}），请使用预置二进制或手动更新。
            </p>
        </t-alert>
