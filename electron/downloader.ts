...
function detectLinuxProxy(): string | null {
    ...
    try {
        ...
        if (mode === "'manual'" || mode === 'manual') {
            ...
            for (const proto of candidates) {
                const host = execSync(`gsettings get org.gnome.system.proxy.${proto} host`, {
                    encoding: 'utf8',
                    timeout: 3000,
                })
                    .trim()
                    .replace(/^'|'$/g, '');
                const port = execSync(`gsettings get org.gnome.system.proxy.${proto} port`, {
                    encoding: 'utf8',
                    timeout: 3000,
                }).trim();
    ...
    // 2. KDE: kreadconfig5
    try {
        const mode = execSync('kreadconfig5 --group Proxy --key ProxyType', {
            encoding: 'utf8',
            timeout: 3000,
        }).trim();
        // KDE ProxyType: 1=手动代理配置
        if (mode === '1') {
            const host = execSync('kreadconfig5 --group Proxy --key HostName', {
                encoding: 'utf8',
                timeout: 3000,
            }).trim();
            const port = execSync('kreadconfig5 --group Proxy --key Port', {
                encoding: 'utf8',
                timeout: 3000,
            }).trim();
            if (host && port && /^\d+$/.test(port)) {
...
export async function startDownload(
    ...
    return new Promise((resolve, reject) => {
        const args = [
            ...
            // 注意：部分 yt-dlp 版本中 progress.percent 为 None（输出 NA），
            // 会导致解析失败、进度不更新。改用 _percent_str 获取可读百分比
            // 字符串（如 "12.3%"），解析时再提取数值。
            '--progress-template',
        ...
        function parseProgress(text: string) {
            ...
            // 1. 优先解析进度模板格式: download: 12.3%|2.34MiB/s|00:05
            // _percent_str 可能带前导空格与百分号，需一并兼容
            const templateMatch = text.match(/^download:\s*([\d.]+)%\s*\|([^|]*)\|([^|\r]*)/);
            if (templateMatch) {
                progress.percent = parseFloat(templateMatch[1]);
                progress.speed = (templateMatch[2] || '').trim();
                progress.eta = (templateMatch[3] || '').trim();
                // 过滤掉 yt-dlp 尚未计算出速度/ETA 时的占位值（如 "Unknown B/s" / "Unknown"）
                if (progress.speed.toLowerCase().startsWith('unknown')) progress.speed = '';
                if (progress.eta.toLowerCase().startsWith('unknown')) progress.eta = '';
                onProgress(progress);
                return;
            }
