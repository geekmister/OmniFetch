...
function cleanupTmp(tmpPath: string) {
    try {
        ...
        console.warn(`[update] 清理临时文件失败: ${tmpPath}`, e);
    }
}
 ...
 */
async function checkOne(name: BinaryName, binPath: string): Promise<UpdateInfo | null> {
    // 系统 PATH 中的命令，不自动更新
    if (binPath === name) {
 ...
 */
export async function applyBinaryUpdates(): Promise<{
    success: boolean;
    updated: BinaryName[];
    failed: BinaryName[];
}> {
